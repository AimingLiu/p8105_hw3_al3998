...
``` r
 ...
 ggplot(aes(x = locationdesc ,y = data_value ,fill = response, group = response))+
 geom_bar(stat = 'identity', position = 'stack') + 
  ...
  labs(
     ...
     x = "locationdesc",
     y = "data_value"
  )+ theme(legend.position = "bottom", plot.title = element_text(color = "red", size = 24))
 ...
 geom_bar(stat = 'identity', position = 'stack') +
  guides(fill = guide_legend(reverse= T))+ 
   ...
   labs(
     ...
     x = "locationdesc",
     y = "data_value"
  )+ theme(legend.position = "bottom",  plot.title = element_text(color = "red", size = 24),plot.tag =element_text(size = 24))
...
help me see the trends.As we can see from the plot.when the `day_id` is
24, 31,the sum of activity count is lowest,which is 1440.and when the
`day_id` is 16,the sum of activity count is largest,which is 685910.
From week one to week two,there is a trend in increasing.In week
three,the fluctuation is to some extent weak and then in week four and
five,the fluctuations are very
dramatic.
...
activity time courses suddenly drops during a specific period of time in
almost everyday in a week which means that during this period of time
the count of activity is small.Also, we can see from the graphic,there
are about 2~3 peaks in `activity_count` which means during these peroid
of time in everyday,the count of activity is larger than count during
other period of time in a day.
